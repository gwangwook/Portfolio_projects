...
{
    public class BackWindowViewModel : ViewModelBase
    {
        ...
        private string _leaveHeaderText;
        private MainWindowViewModel _mainViewModel;

        public BackWindowViewModel(MainWindowViewModel mainViewModel)
        {
            _mainViewModel = mainViewModel;
            StartBackWebcamCommand = ReactiveCommand.CreateFromTask(StartBackWebcamAsync);
            StopBackWebcamCommand = ReactiveCommand.Create(StopBackWebcam, Observable.Return(true));

            mainViewModel.WhenAnyValue(
                vm => vm.StartText,
                vm => vm.ReturnText,
                vm => vm.QRCodeResultText,
                vm => vm.LeaveHeaderText)
                .Subscribe(tuple =>
                {
                    StartText = tuple.Item1;
                    ReturnText = tuple.Item2;
                    QRCodeResultText = tuple.Item3;
                    LeaveHeaderText = tuple.Item4;
                });
        }
        ...
        public ReactiveCommand<Unit, Unit> StartBackWebcamCommand { get; }
        public ReactiveCommand<Unit, Unit> StopBackWebcamCommand { get; }

        private async Task StartBackWebcamAsync()
        {
                                ...
                                while (true)
                                {
                                    ...
                                    if (response == "Face True")
                                    {
                                        ...
                                        await Dispatcher.UIThread.InvokeAsync(() =>
                                        {
                                            ...
                                            var backWindow = applicationLifetime?.Windows.OfType<BackWindow>().FirstOrDefault();
                                            backWindow?.ReturnToMainWindow();
                                        });
        ...
        private async Task<bool> ProcessAndSendImage(string emp_number)
        {
            ...
            while (_isRunning)
            {
                ...
                if (!_frame.IsEmpty)
                {
                    ...
                    int countdown = 6 - (int)elapsedSeconds;
                ...
                if ((DateTime.Now - startTime).TotalSeconds > 6 && !captured)
                {
